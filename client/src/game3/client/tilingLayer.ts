...
export class TilingLayer{
  ...
  updataCache(camera:Vector, tileSize:number){
    ...
    const renderTileHeight = Math.min(mapHeight, viewTileHeight);
    const renderTileWidth = Math.min(mapWidth, viewTileWidth);

    ct.clearRect(0, 0, copier.width, copier.height);
