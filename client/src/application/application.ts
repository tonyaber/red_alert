...
import { Settings, IGameOptions } from "./settingsPageMulti2";
...
export class Application extends Control {
  ...
  singleCycle(res: Record<string, HTMLImageElement>) {
    ...
    settings.animateIn().then(_ => {
      settings.onPlay = (set) => {
        // передаем set параметры для настройки игры
        res.map = set.map;
        console.log(res.map);
        console.log(set.map);
        ...
        roomPage.onCreateGame = () => {
          ...
          settings.animateIn().then((_) => {
            settings.onCreate = (data) => {
              ...
              settings.animateOut().then((val) => settings.destroy());
            };
          });
        };
        roomPage.onStartGame = (data) => {
          //при мульти ждет игроков
  ...
  gameCycle(name: string, data: any, res: Record<string, HTMLImageElement>) {
    ///TODO type??
